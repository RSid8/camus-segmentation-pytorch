"""
Taken from https://github.com/BioMedIA-MBZUAI/contrastive-echo/blob/main/scripts/camus/camus_dataset.py
"""
import os
import numpy as np
import torch
...
class Camus(Dataset):
    def __init__(
        self, root=None, split="train", global_transforms=[], augment_transforms=[]
    ):
    ...
    def __read_image(self, patient_file, suffix):
        image_file = "{}/{}/{}".format(
            self.data_dir, patient_file, patient_file + suffix
        )
    ...
    def __getitem__(self, index):
        ...
        info_2CH = self.__read_info(
            "{}/{}/{}".format(self.data_dir, patient_file, "Info_2CH.cfg")
        )
        info_4CH = self.__read_info(
            "{}/{}/{}".format(self.data_dir, patient_file, "Info_4CH.cfg")
        )
